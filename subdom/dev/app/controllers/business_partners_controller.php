...
class BusinessPartnersController extends AppController {
	...
	function user_view($id = null) {
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSStorings/index')) {
			...
			// pokud chci vysledky vyhledavani
			if ( isset($this->data['CSStoring']['search_form']) && $this->data['CSStoring']['search_form'] == 1 ){
				$this->Session->write('Search.CSStoringForm', $this->data);
				$c_s_storings_conditions = $this->BusinessPartner->CSStoring->do_form_search($c_s_storings_conditions, $this->data);
			} elseif ($this->Session->check('Search.CSStoringForm')) {
				$this->data = $this->Session->read('Search.CSStoringForm');
				$c_s_storings_conditions = $this->BusinessPartner->CSStoring->do_form_search($c_s_storings_conditions, $this->data);
			}
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSInvoices/index')) {
			...
			if ( isset($this->data['CSInvoice']['search_form']) && $this->data['CSInvoice']['search_form'] == 1 ){
				...
				$c_s_invoices_conditions = $this->BusinessPartner->CSInvoice->do_form_search($c_s_invoices_conditions, $this->data);
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSCreditNotes/index')) {
			...
			// pokud chci vysledky vyhledavani
			if (isset($this->data['CSCreditNote']['search_form']) && $this->data['CSCreditNote']['search_form'] == 1) {
				$this->Session->write('Search.CSCreditNoteForm', $this->data);
				$c_s_credit_notes_conditions = $this->BusinessPartner->CSCreditNote->do_form_search($c_s_credit_notes_conditions, $this->data);
		...
		if ($this->Acl->check(array('model' => 'User', 'foreign_key' => $this->Session->read('Auth.User.id')), 'controllers/CSTransactions/index')) {
			...
			if ( isset($this->data['CSTransaction']['search_form']) && $this->data['CSTransaction']['search_form'] == 1 ){
				...
				$c_s_transactions_conditions = $this->BusinessPartner->CSTransaction->do_form_search($c_s_transactions_conditions, $this->data);
			} elseif ($this->Session->check('Search.CSTransactionForm')) {
				$this->data = $this->Session->read('Search.CSTransactionForm');
				$sales_conditions = $this->BusinessPartner->CSTransaction->do_form_search($c_s_transactions_conditions, $this->data);
			}
